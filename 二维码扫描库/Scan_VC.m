...
-(void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection{
    if (metadataObjects.count>0) {
        ...
        if ([result isEqual:[NSNull null]]) {
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"扫描结果" message:@"二维码扫描失败，请确认。" delegate:self cancelButtonTitle:nil otherButtonTitles:@"再次扫描", nil];
            [alert show];
        }else{
            
           
            [self AnalyString:result];
...
-(void)AnalyString:(NSString *)result
{
